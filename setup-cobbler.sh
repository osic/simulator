...
pushd /var/cache/iso
  if [ -f "/var/cache/iso/ubuntu-14.04.5-server-amd64.iso" ]; then
    rm /var/cache/iso/ubuntu-14.04.5-server-amd64.iso
  fi
...
if ! cobbler distro list | grep -qw "ubuntu-14.04.4-server-x86_64"; then
  mkdir_check "/mnt/iso"
  mount -o loop /var/cache/iso/ubuntu-14.04.5-server-amd64.iso /mnt/iso
